...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
    ...
    {
        for(int a = 0; a < strlen(sentence_ptr); a++) {
            if((strlen((sentence_ptr + a)) - strlen(searchWord_ptr)) >= 0) {
                for(int b = 0; b < strlen(searchWord_ptr); b++) {
                    if(*(sentence_ptr + a + b) != *(searchWord_ptr + b)) {
                        break;
                    }
                    else if(b == strlen(searchWord_ptr) - 1) {
                        *errorCode_ptr = ERROR_CODE_SUCCESS;
                        return &sentence_ptr[a];
                    }
                }
            }
        }
